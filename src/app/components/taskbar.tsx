...
const Taskbar = ({ openWindows, restoreApp, minimizeApp, minimized, focusedApp }) => {
    return (
        <div className="fixed z-[1000] h-[70px] pr-4 pl-4 bottom-0 w-full bg-transparent backdrop-blur-2xl text-white flex justify-center items-center p-2 transition-all">
            ...
            <div className="ml-auto w-fit flex flex-row justify-center items-center">
                <BatteryStatus />
                <p className="w-fit" suppressHydrationWarning>{new Date().toLocaleTimeString()}</p>
            </div>
        </div>
    );
};
